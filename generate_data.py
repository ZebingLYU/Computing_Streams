    ...
    for day in range(num_days):
        ...
        with open(file_path, 'w') as file:
            ...
            for _ in range(num_rows_per_day - file.tell()):
                ...
                # Occasionally generate a corrupted row
                if random.random() < 0.01:  # 1% chance of a corrupted row
                    corruption_type = random.choice(range(4))
                    if corruption_type == 0:
                        corrupted_row = f"{sng_id} | {user_id}\n"  # Missing field
                    elif corruption_type == 1:
                        corrupted_row = f"{sng_id} | abc | {country}\n"  # Non-Numeric user_id
                    elif corruption_type == 2:
                        corrupted_row = f"abc | {user_id} | {country}\n"  # Non-Numeric sng_id
                    elif corruption_type == 3:
                        corrupted_row = f"{sng_id} | {user_id} | ??\n" #invalide country

                    file.write(corrupted_row)
                else:
